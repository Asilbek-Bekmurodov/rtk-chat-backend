...
// Chat ichidagi 1 ta xabarni tahrirlash
router.patch("/:chatId/messages/:messageId", verifyToken, async (req, res) => {
  const { chatId, messageId } = req.params;
  const { text } = req.body;

  if (!text || typeof text !== "string" || !text.trim()) {
    return res.status(400).json({ message: "Text kerak" });
  }
  ...
  const isOwner = message.sender?.toString() === req.user.id;
  if (!isAdmin && !isOwner) {
    return res.status(403).json({ message: "Faqat egasi tahrirlay oladi" });
  }
